jQuery(document).ready(function ($) {
    ...
    $(document).ready(function() {
		 ...
		 {
		 var min = Number(get_filter('price'));
		 if(min == 0){
			 var max = 200000000;
		 }
		 else if(min == 10000000 || min == 20000000){
			 var max = min + 10000000; 
		 }else if(min == 30000000){
			 var max = min + 100000000;
		 }else{
			 var max = min + 5000000;
		 }
		 
		 
		 var lsp = get_filter('lsp');
		 if(lsp < 1){
			 lsp = 0;
		 }
		 var th = get_filter('thuongh');
		 if(th < 1){
			 th = 0;
		 }
		 var lm = get_filter('loaim');
		 if(lm < 1){
			 lm = 0;
		 }
		 var cn = get_filter('congnghe');
		 if(cn < 1){
			 cn = 0;
		 }
		 $.ajax({
		 method :"GET",
		 url:'/search/'+min +"/" + max + "/" + lm + "/" + lsp+ "/" + th+ "/" + cn,
//		 data:{min:min, max:max,
//		 lsp:loaisanpham, th:thuonghieu, lm:loaimay,cn:congnghe},
		 success: function(products) {
			 var s = '';
			 var leng = products.length;
			 for (var i = 0; i < leng; i++) {
				s += '<div class="product-box">';
				s += '<div class="product-box-content">';
				s += '<figure class="img-product">';
				s += '<img style="width: 202px; height: 239px;" src="/images/' + products[i].fileImage +'" alt="">';
				s += '<a href="product/' + products[i].id +'" class="flaticon-search"></a>';
				s += '</figure>';
				s += '<div class="product-box-text">';
				s += '<a href="product/' + products[i].id + '" class="product-name">'+products[i].name +'</a>';
				s += '<a href="#" class="write">Write Your Review</a>';
				s += '<p class="product-cost">'+ Intl.NumberFormat().format(products[i].price) +' đ' +'</p>';
				s += '<p class="desc-product"></p>';
				s += '<div class="product-box-bottom">';
				s += '<a href="#" class="add-to-cart"><i class="flaticon-commerce"></i>Add To Cart</a>';
				s += '<a href="#" class="wishlist"><i class="flaticon-like"></i></a>';
				s += '<a href="#" class="refresh-product"><i class="flaticon-arrows"></i></a>';
				s += '</div>';
				s += '</div>';
				s += '</div>';
				s += '</div>';
			}
			 $('.products-categories').html(s);
		}
		 });
		 };
...
//		                $(this).find('.amount').html((ui.values[0]/1000000).toPrecision(2) + 'Trieu');
//		                $(this).find('.amount2').html((ui.values[1]/1000000).toPrecision(2) + 'Trieu');
//		                $('#hidden_minimum_price').val(ui.values[0]);
//			            $('#hidden_maximum_price').val(ui.values[1]);
//			            filter_data();
//		            }
//		        });
//		        $(this).find('.amount').html($(this).slider('values', 0).toPrecision(2) + 'Trieu');
//		        $(this).find('.amount2').html($(this).slider('values', 1)/1000000 + 'Trieu');
//		        
//		        
//		    });
